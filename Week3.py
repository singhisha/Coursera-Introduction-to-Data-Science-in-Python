...
def answer_one():
    ...
    energy = energy.loc[18:243,2:6]
    energy.columns = ['Country', 'Energy Supply', 'Energy Supply per Capita', '% Renewable']
    energy[energy=='...']=np.nan
    energy['Energy Supply'] *= 1000000 

    def remove_digit(d):
        new_d = ''.join([i for i in str(d) if not i.isdigit()])
        i = new_d.find('(')
        if i>-1: new_d = new_d[:i]
        return new_d.strip()
...
def answer_two():
    ...
    energy.replace('Republic of Korea','South Korea',inplace = True)
    energy.replace("United States of America", "United States",inplace = True)
    energy[energy['Country']=='United States of America'] = 'United States'
    energy.replace('United Kingdom of Great Britain and Northern Ireland','United Kingdom',inplace = True)
    energy.replace('China, Hong Kong Special Administrative Region', 'Hong Kong',inplace = True)
    def remove_digit(d):
        new_d = ''.join([i for i in str(d) if not i.isdigit()])
    ...
    inter = pd.merge(pd.merge(energy, GDP, on='Country'), ScimEn, on='Country')   
...
def answer_four():
    ...
    diff = abs(Top15.iloc[5]['2015'] - Top15.iloc[5]['2006'])
    return diff
answer_four()
...
def answer_five():
    Top15 = answer_one()
    avgEnergy = Top15['Energy Supply per Capita'].mean()
    return avgEnergy
answer_five()
...
def answer_six():
    ...
    maxi = Top15.sort_values(by='% Renewable', ascending = False).iloc[0]
    return (maxi.name, maxi['% Renewable'])
answer_six()
...
def answer_seven():
    ...
    maxi =  Top15.sort_values(by= 'Ratio' , ascending = False).iloc[0]
    return (maxi.name, maxi['Ratio'])
answer_seven()
...
def answer_eight():
    ...
    maxi =  Top15.sort_values(by= 'Population' , ascending = False).iloc[2]
    return maxi.name
answer_eight()
...
def answer_nine():
    ...
    c = Top15['Energy Supply per Capita'].corr(Top15['Citable docs per Capita'])
    return c
answer_nine()
...
def answer_ten():
    ...
    Top15['Bool'] = Top15['% Renewable']>=med
    Top15['Bool'] = Top15['Bool'].apply(lambda x:1 if x else 0)
...
def answer_eleven():
    ...
    Top15['Population'] = Top15['Energy Supply'] / Top15['Energy Supply per Capita']
    Top15new = pd.DataFrame(columns = ['size', 'sum', 'mean', 'std'])
    for group, frame in Top15.groupby('Continent'):
        Top15new.loc[group] = [len(frame), frame['Population'].sum(),frame['Population'].mean(),frame['Population'].std()]
    return Top15new
answer_eleven()
...
def answer_twelve():
    ...
    groups = Top15.groupby(['Continent','Bins'])
    return groups.size()
answer_twelve()
...
def answer_thirteen():
    ...
    PopEst = Top15['Population Estimate'].map('{:,}'.format)
    return PopEst
answer_thirteen()
